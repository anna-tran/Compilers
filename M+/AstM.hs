...
instance Show M_stmt where
    ...
    show (M_while (expr, stmt))           = indent $ "M_while (" ++ (show expr) ++ ",\n" ++ (show stmt) ++ ")"
    show (M_cond (expr, stmt1, stmt2))    = indent $ "M_cond (" ++ (show expr) ++ ",\n" ++ (show stmt1) ++ ",\n" ++ (show stmt2) ++ ")"
...
instance Show M_expr where
    ...
    show (M_app (op, exprs))      = indent $ "M_app (" ++ (show op) ++ "," ++ (showMyList exprs) ++ ")"
